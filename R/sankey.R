                            ...
                            const.bin.size = TRUE)
{
    ...
    .constructNodeName <- function(node, i, i.parent, frame, tree.hash, model)
    {
        ...
        else
        {
            ...
            # split node string by 1 or 2 characters
            node.str <- if (nchar(node.name) == 1 || is.na(suppressWarnings(as.numeric(substr(node.name, 2, 2)))))
                strsplit(node.name,"")
            else
                list(substring(node.name, first = seq(1, nchar(node.name), 2), last = seq(2, nchar(node.name), 2)))
